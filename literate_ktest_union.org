...
distance_matrix = np.zeros((len(examples),) * 2, dtype=int)
for left, right in combinations(range(len(examples)), 2):
    distance_matrix[left, right] = examples[left].distance(examples[right])
    distance_matrix[right, left] = distance_matrix[left, right]
#+END_SRC
...
def learn_ktest_union(examples, k):
    ...
    for i in range(1, len(examples) - 1):
        ...
        for j in range(i + 1, len(examples)):
            ...
            neighbours.sort(key=attrgetter('dist'))
...
# instanciated = [ktestable.from_example(ex, 3) for ex in paper_dataset]
# from itertools import product

# print(list(product(range(len(instanciated)))))
# matrix = np.zeros((len(instanciated),) * 2, dtype=bool)
# for left in range(len(instanciated)):
#     for right in range(len(instanciated)):
#         matrix[left, right] = instanciated[left].is_union_consistent_with(instanciated[right])
# print(matrix)
#+END_SRC

#+RESULTS: learnktest
#+BEGIN_example
[(<__main__.ktestable object at 0x7f33996f2a90>, 0), (<__main__.ktestable object at 0x7f3391a9ec88>, 7), (<__main__.ktestable object at 0x7f33996f2da0>, ((1, 4), 6)), (<__main__.ktestable object at 0x7f33996f22b0>, ((3, 5), 2))]
#+END_example
