...
We propose a way to simplify this computation.

There are only two ways a path can form between red and blue ; via a direct path and via an indirect path.
We only consider paths from blue to red but the same applies to paths from red to blue.
A direct path is when an edge exists between a blue and a red node, whereas an indirect path goes from to a blue to a read node, while going through $n$ white nodes, $n \in \mathbb{N}_{\textgreater 0}$.
...
We will make use of the defaultdict class :
#+BEGIN_SRC ipython
...
We create the test script =coherence_tests_v2= in order to make sure that we have the same results across all our methods.
