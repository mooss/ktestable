...
A direct path is when an edge exists between a blue and a red node, whereas an indirect path goes from to a blue to a red node, while going through $n$ white nodes, $n \in \mathbb{N}_{> 0}$.
...
Below is an extract of a line by line profile, obtained with the module =line_profiler=[fn:: See https://pypi.org/project/line_profiler/], on the initial version of =ktestable=.
#+BEGIN_SRC ipython
